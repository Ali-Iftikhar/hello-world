...
fprintf ('from time t=0 to t = %-3.0f \n', k * dt)
fprintf ('The stepsize is Delta_t = %2.12g\n', dt)
...
for q = 1 : m
    ...
    while ttt<tmax && e>error
        ...
        for j = 1: n*k
            if order == 4 % This is the 4th order RK method
                ...
                ta = j*h;
            elseif order == 5 % This is the 5th order RK method
                k1 = h*f(ya);
                k2 = h*f(ya + k1/2);
                ...
                fprintf('The order of the method must be 4 or 5\n')
                break 
            end % end if
    ...
    hold on
    a = 0.1; b = 0.1; c = 14;
    A1 = c + sqrt(c^2-4*a*b); A2 = -c-sqrt(c^2-4*a*b);
    A3 = c-sqrt(c^2-4*a*b); A4 = -c + sqrt(c^2-4*a*b);
    plot3(A1/2, A2/(2*a), A1/(2*a), 'r*') % plots fixed points
    hold on
    plot3(A3/2, A4/(2*a), A3/(2*a), 'r*') % plots fixed points
    axis (plotrange);
    grid on;
end
